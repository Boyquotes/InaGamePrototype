...
func unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("summon_tako"):
		pass
...
func physics_process(delta: float) -> void:
	...
	if player.is_on_floor() and _state_machine._state_name != "Air":
		var floor_normal = player.get_floor_normal()
		_snap = -floor_normal * Vector3(0,0.2,0)
		velocity += floor_normal
	
	# Movement
	velocity = calculate_velocity(velocity, move_direction, delta)
	velocity = player.move_and_slide_with_snap(velocity, _snap, Vector3.UP, true)
